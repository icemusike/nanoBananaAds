    ...
    const clientsWithStats = await Promise.all(
      clients.map(async (client) => {
        const [projectsCount, brandsCount, adsCount] = await Promise.all([
          ...
          prisma.ad.count({ where: { agencyClientId: client.id } })
        ]);
...
router.get('/clients/:id', checkAgencyLicense, async (req, res) => {
  try {
    ...
    const [projectsCount, brandsCount, adsCount] = await Promise.all([
      ...
      // Count ads generated by this client
      prisma.ad.count({
        where: { agencyClientId: id }
      })
    ]);
  ...
  try {
    ...
    const ads = await prisma.ad.findMany({
      ...
      select: {
        ...
        aspectRatio: true,
        imageMimeType: true,
        // Don't include imageData in list for performance
        imageData: false
      }
    });
